 ...
 * - Listing printers and their current status
 * - Tracking project workflows
 * - Managing customers
 * - Monitoring inventory
 * - Accessing documents (quotations, invoices, etc.)
 * - Retrieving costing templates
 */
...
import {
    ...
    OrderSchema,
    DocumentSchema,
    CostingTemplateSchema
} from './schemas';
// Type override since this file is only used in development
type AITool = { 
  name: string; 
  description: string; 
};
...
interface AIOverride {
  defineTool: (config: { 
    ...
    inputSchema?: any; 
    outputSchema?: any; 
  }, handler: (input: any) => Promise<any>) => AITool;
  generate: (config: { 
    messages: MessageData[];
    tools?: AITool[];
    systemPrompt?: string;
    model?: string;
  }) => Promise<{ text: string; toolRequests?: any[] }>;
}

const aiOverride = ai as unknown as AIOverride;

// --- Tool Definitions ---

const listPrintersTool = aiOverride.defineTool(
  {
    name: 'listPrinters',
    description: 'Get a list of all 3D printers in the fleet, including their real-time status and current job.',
    outputSchema: z.array(PrinterSchema),
  },
...
const getProjectWorkflowTool = aiOverride.defineTool(
    {
...
const listCustomersTool = aiOverride.defineTool(
    {
        name: 'listCustomers',
    ...
    async (input: { name?: string }) => {
        ...
        if (input.name) {
            const filtered = customers.filter(c => input.name ? c.name.toLowerCase().includes(input.name.toLowerCase()) : true);
...
const getInventoryStatusTool = aiOverride.defineTool(
    {
    ...
    async (input: { type?: string; status?: string; itemName?: string; }) => {
...
const getOrdersTool = aiOverride.defineTool(
    {
        name: 'getOrders',
    ...
    async (input: { customerName?: string; orderNumber?: string; status?: string }) => {
        let orders = generateInitialOrders();
        
        if (input.customerName) {
            orders = orders.filter(o => input.customerName ? o.customer.toLowerCase().includes(input.customerName.toLowerCase()) : true);
        }
        
        if (input.orderNumber) {
            orders = orders.filter(o => input.orderNumber ? o.orderNumber.toLowerCase() === input.orderNumber.toLowerCase() : true);
        }
...
const getDocumentsTool = aiOverride.defineTool(
    {
        name: 'getDocuments',
    ...
    async (input: { customerName?: string; documentType?: string }) => {
        ...
        if (input.customerName) {
            ...
            if (customer) {
                // Assume documents have a relation to customers via customerId
                // If the Document type doesn't have customerId, just comment out the filtering
                // documents = documents.filter(d => d.customerId === customer.id);
                
                // For now, return all documents since we can't filter by customer
            } else {
        ...
        if (input.documentType) {
            documents = documents.filter(d => d.docType === input.documentType);
        }
        
        return documents.map(doc => {
            // Since we don't have customerId in the Document type, we'll assign a placeholder
            // const customer = customers.find(c => c.id === doc.customerId);
            return {
                orderNumber: doc.id, // Use document ID as orderNumber since it might not have orderNumber
                customerName: 'Unknown', // Can't link to customer without customerId
                type: doc.docType,
                date: doc.createdAt,
                amount: doc.amount
            };
...
const getCostingTemplatesTool = aiOverride.defineTool(
    {
    ...
    async (input: { templateName?: string }) => {
        let templates = generateInitialCostingTemplates(); // This would fetch from a database or state management

        if (input.templateName) {
            templates = templates.filter(t => t.name && t.name.toLowerCase().includes(input.templateName!.toLowerCase()));
        }
        
        return templates.map(t => ({
            id: t.id || '',
            name: t.name || 'Unknown Template',
            jobName: t.inputs?.jobName || 'Unknown Job',
            printTime: `${t.inputs?.printHours || 0}h ${t.inputs?.printMinutes || 0}m`,
            filamentWeight: Number(t.inputs?.filamentWeight || 0),
        }));
    }
);
...
export async function getDocuments(input: GetDocumentsInput) {
    const documents = generateInitialDocuments();
    const customers = generateInitialCustomers();

    let filteredDocuments = documents;
    if (input.customerName) {
        const customer = customers.find(c => input.customerName ? c.name.toLowerCase().includes(input.customerName.toLowerCase()) : false);
        if (customer) {
            // Since we don't have customerId in the Document type, we'll skip filtering
            // filteredDocuments = documents.filter(d => d.customerId === customer.id);
            filteredDocuments = documents; // Using all documents for now
        } else {
            return [];
        }
    }
    ...
    if (input.documentType) {
        filteredDocuments = filteredDocuments.filter(d => d.docType === input.documentType);
    }
    
    return filteredDocuments.map(doc => {
        // Since we don't have customerId in the Document type
        // const customer = customers.find(c => c.id === doc.customerId);
        return {
            orderNumber: doc.id, // Use document ID as orderNumber
            customerName: 'Unknown', // No way to link to customer without customerId
            type: doc.docType,
            date: doc.createdAt,
            amount: doc.amount
        };
...
export async function chat(input: { prompt: string, history?: { role: 'user' | 'model', content: string }[] }) {
  ...
  const result = await aiOverride.generate({
    model: 'googleai/gemini-1.5-flash-latest',
    messages: [...history, { role: 'user', content: [{ text: input.prompt }] }],
    tools: allTools,
    systemPrompt: "You are a helpful assistant for a 3D printing farm. You have access to a comprehensive set of tools to query real-time data about the application. Use these tools to answer user questions about printers, project workflows, customers, orders, documents (like invoices), and inventory. When asked for structured data, ALWAYS format your response as a Markdown table for clarity. Use lists for enumerations and bold text for emphasis to make your responses as clear and interactive as possible."
  });
