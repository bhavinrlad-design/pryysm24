...
import type { Job, Printer, Document, InventoryItem, StockStatus, InventoryCategory, Customer, Spool, Resin, Powder, SavedCalculation, LoggedCalculation, Currency, MaterialStatus } from './workspace';
...
export const generateInitialPrinters = (): Omit<Printer, 'completionEstimate'|'idleSince'|'utilization'|'currentJob'|'currentJobImage'>[] => [
    ...
    { id: '2', name: 'Creality Ender 3 Pro', model: 'Ender 3 Pro', codeName: 'ENDER01', location: 'Lab 2', technology: 'FDM', initializationDate: new Date('2023-03-22'), capacity: 'Standard', material: 'PLA', status: 'Online' },
...
export const generateInitialOrders = (): any[] => {
    ...
    // Reduced from 60 to 20 orders for initial load
    for (let i = 1; i <= 20; i++) {
...
export const generateInitialRawMaterials = () => {
    ...
    const initialSpools: Spool[] = [
        ...generateSpools(8, "PLA", "Overture", "Black", "#000000", "Matte", "PLA Black", 1, "Rack A"),
        ...generateSpools(6, "PLA", "Hatchbox", "White", "#FFFFFF", "Glossy", "PLA White", 21, "Rack B"),
        ...generateSpools(5, "ABS", "Sunlu", "Red", "#FF0000", "Satin", "ABS Red", 36, "Rack C"),
        ...generateSpools(4, "PETG", "eSun", "Blue", "#0000FF", "Transparent", "PETG Blue", 46, "Rack D"),
        ...generateSpools(2, "TPU", "NinjaFlex", "Grey", "#808080", "Flexible", "TPU Grey", 56, "Rack E"),
    ];

    const initialResins: Resin[] = [
        ...Array.from({length: 5}, (_, i) => ({ id: 104 + i, resinId: `RS${String(4+i).padStart(3, '0')}`, name: 'Standard Resin Grey', brand: 'Elegoo', color: '#808080', type: 'Standard', volume: 1000, used: (i % 10) * 100, price: 35.00, currency: 'USD' as Currency, purchaseDate: '2024-05-10', notes: "Stock", status: 'Active' as any, assignedToPrinterId: undefined, assignedToJobId: undefined, minStock: 2, minOrder: 4, location: `Resin Cabinet ${i % 3 + 1}` })),
...
const getColorForDuration = (durationHours: number): string => {
    ...
    if (durationHours <= 24) return '#E6A635'; // Accent Amber
    return '#DC2626'; // Error Red for long jobs
};
...
const generateBusySchedule = () => {
    ...
    for (const printer of printers) {
        ...
        for (let i = 0; i < 15; i++) { // Reduced from 40 to 15 jobs per printer
...
export const generateInitialCostingTemplates = (): Partial<SavedCalculation>[] => [
    {
        id: 'template-1', name: 'Standard PLA Part (120g)', createdAt: new Date('2024-05-01T10:00:00Z').toISOString(),
        inputs: { 
            jobName: 'Standard PLA Part', 
            productImage: 'https://placehold.co/100x100/3B82F6/FFFFFF/png', 
            currency: 'USD', 
            printHours: 5, 
            printMinutes: 30, 
            filamentWeight: 120, 
    ...
    },
    {
        ...
        inputs: { 
            jobName: 'Quick ABS Prototype', 
            productImage: 'https://placehold.co/100x100/EF4444/FFFFFF/png', 
            currency: 'USD', 
            printHours: 2, 
            printMinutes: 0, 
            filamentWeight: 50, 
    ...
    },
    {
        ...
        inputs: { 
            jobName: 'Detailed Resin Miniature', 
            productImage: 'https://placehold.co/100x100/8B5CF6/FFFFFF/png', 
            currency: 'EUR', 
            printHours: 4, 
            printMinutes: 0, 
            filamentWeight: 20, 
...
export const generateInitialLoggedCalculations = (): Partial<LoggedCalculation>[] => [
    {
        id: 'log-1', 
        name: 'Standard PLA Part',
        createdAt: new Date('2024-05-15T11:00:00Z').toISOString(),
        inputs: generateInitialCostingTemplates()[0].inputs as any,
        pricingInputs: generateInitialCostingTemplates()[0].pricingInputs as any,
        result: {
            costPerUnit: 29.97,
            materialCost: 3,
            laborCost: 26.67,
            electricityCost: 0.16,
            failureRateCost: 0.01,
            customCosts: 0,
            totalOverhead: 0.13,
            profitAmount: 12,
            sellingPrice: 42,
    ...
    {
        id: 'log-2', 
        name: 'Gear Prototype V2',
        createdAt: new Date('2024-05-14T16:45:00Z').toISOString(),
        inputs: generateInitialCostingTemplates()[1].inputs as any,
        pricingInputs: generateInitialCostingTemplates()[1].pricingInputs as any,
        result: {
            costPerUnit: 12.09,
            materialCost: 1.5,
            laborCost: 10.42,
