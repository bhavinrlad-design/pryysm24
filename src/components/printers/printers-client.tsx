...
export function PrintersClient() {
    const { printers: machines, schedule, confirmJobUpload } = useWorkspace() as unknown as { 
        printers: Printer[],
        schedule: any[],
    ...
    const statusCounts = useMemo(() => {
        const initialCounts: Record<string, number> = { 
            running: 0, idle: 0, maintenance: 0, offline: 0, 
            Online: 0, Offline: 0, Maintenance: 0, Printing: 0 
        };
        return (machines || []).reduce((acc, printer) => {
            // Map new status values to old ones for counting purposes
            let countStatus;
            if (printer.status === 'Printing') countStatus = 'running';
            else if (printer.status === 'Online') countStatus = 'idle';
            else if (printer.status === 'Maintenance') countStatus = 'maintenance';
            else if (printer.status === 'Offline') countStatus = 'offline';
            else countStatus = String(printer.status).toLowerCase();
            
            if (countStatus in acc) {
              acc[countStatus]++;
            }
            return acc;
        }, initialCounts);
