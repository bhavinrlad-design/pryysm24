...
import type { Printer, PrinterStatus, PrinterTechnology } from '../../types/workspace'
...
export function AddRemovePrinterClient() {
  ...
  // Comment out for now as these properties may not be available
  // const { printers, addPrinter, deletePrinter, updatePrinterStatus, idService } = useWorkspace();
  
  // Create placeholders for the properties we expect from useWorkspace
  const printers: Printer[] = [];
  const addPrinter = (printer: Omit<Printer, 'id'>) => ({ ...printer, id: Math.random().toString() } as Printer);
  const deletePrinter = (id: string) => {};
  const updatePrinterStatus = (id: string, status: PrinterStatus) => {};
  const idService = { 
    generate: () => Math.random().toString(),
    getNextId: (prefix: string) => `${prefix.toUpperCase()}-${Math.floor(Math.random() * 1000).toString().padStart(3, '0')}`
  };
  ...
  const handleAddPrinter = (e: React.FormEvent) => {
    ...
    const newPrinter: Omit<Printer, 'id'> = {
      ...
      capacity,
      material,
      status: 'idle', // Add the missing status field
                ...
                <CardContent>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {filteredPrinters.map(printer => {
                            ...
                            return (
                             <Card key={printer.id} className={`${statusBorderColors[printer.status as PrinterStatus]}`}>
                                <CardHeader className="p-4">
                                    <div className="flex items-start justify-between">
                                        ...
                                        <div className={`text-xs font-bold capitalize px-2 py-0.5 rounded-full ${statusTextColors[printer.status as PrinterStatus]}`}>
                                            {printer.status}
                                        </div>
                                    </div>
                                    {printer.currentJob && (
