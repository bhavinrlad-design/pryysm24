...
import { PlusCircle, Search, Users, FileText, ShoppingCart, Receipt, Edit, Eye, LogOut, User, X, Loader2, Package } from 'lucide-react'
...
import type { Customer, Document, DocumentType } from '../../hooks/workspace'
...
export function CustomersClient() {
  ...
  const { customers, documents: allDocuments, addCustomer, updateCustomer } = useWorkspace() as any;
  ...
  const documents = useMemo(() => {
    ...
    if (selectedCustomer) {
        filteredDocs = filteredDocs.filter((doc: Document) => doc.customerId === selectedCustomer.id);
    }
    
    if (docTypeFilter !== 'All') {
        filteredDocs = filteredDocs.filter((doc: Document) => doc.docType === docTypeFilter);
    }

    if (docSearchQuery) {
        const lowercasedQuery = docSearchQuery.toLowerCase();
        const customerMap = new Map<string, string>(customers.map((c: Customer) => [c.id, c.name]));
        filteredDocs = filteredDocs.filter((doc: Document) => {
            const customerName = (customerMap.get(doc.customerId || '') || '').toLowerCase();
            return (doc.orderId || '').toLowerCase().includes(lowercasedQuery) || customerName.includes(lowercasedQuery);
        });
    }
  ...
  const getCustomerById = (id?: string) => id ? customers.find((c: Customer) => c.id === id) : undefined;
                  ...
                  <TableBody>
                    {documents.length > 0 ? (
                        documents.map((doc: Document) => {
                          const Icon = docTypeIcons[doc.docType as DocumentType];
                          ...
                          return (
                            <TableRow key={doc.id}>
                              <TableCell>
                               ...
                               <TableCell className="font-medium">{doc.orderName || doc.orderId || 'No ID'}</TableCell>
                              <TableCell>{customer?.name || 'N/A'}</TableCell>
                              <TableCell>{doc.createdAt ? format(new Date(doc.createdAt), 'dd-MM-yyyy') : 'No date'}</TableCell>
