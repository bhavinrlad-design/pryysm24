...
import type { KanbanColumn as KanbanColumnType, KanbanCardData, KanbanColumnId } from './types';
...
const KanbanCard = React.forwardRef<HTMLDivElement, KanbanCardProps>(({ item, sourceColumnId, isHighlighted, onClick }, ref) => {
    ...
    const priorityClasses = {
        high: { border: 'border-l-error', bg: 'bg-error/10 text-error' },
        medium: { border: 'border-l-warning', bg: 'bg-warning/10 text-warning' },
        low: { border: 'border-l-success', bg: 'bg-success/10 text-success' },
    };
        ...
        >
            <div className="flex items-start gap-4">
                ...
                <div className="flex-grow min-w-0">
                    <p className="font-semibold text-sm leading-tight truncate">{typeof customer === 'string' ? customer : customer?.name}</p>
            ...
            <div className="mt-4 pt-3 border-t grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
                ...
                <div className="flex items-center gap-1.5">
                 ...
                 <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-semibold">{deadline ? format(new Date(deadline), 'dd-MMM') : 'N/A'}</span>
                </div>
            </div>
        </div>
    );
