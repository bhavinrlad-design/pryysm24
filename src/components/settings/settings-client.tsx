...
import { type CodeSettings, type Document } from '../../hooks/workspace'
...
export function SettingsClient() {
    ...
    const { workspaceData, documents, customers, codeSettings: initialCodeSettings, updateCodeSettings, supportTickets, addSupportTicket } = useWorkspace() as unknown as {
        workspaceData: any,
        documents: Document[],
        customers: any[],
    ...
    const handleCodeSettingChange = (key: string, value: string) => {
        setCodeSettings((prev: any) => ({...prev, [key]: value}));
    };
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                {Object.keys(codeSettings).map((key) => (
                                    <div className="space-y-2" key={key}>
                                        <Label htmlFor={`code-${key}`} className="capitalize">{key.replace(/([A-Z])/g, ' $1')}</Label>
                                        <Input
                                            id={`code-${key}`}
                                            value={String(codeSettings[key as keyof typeof codeSettings])}
                                            onChange={(e) => handleCodeSettingChange(key, e.target.value)}
                                            placeholder={String(initialCodeSettings[key as keyof typeof initialCodeSettings])}
                                        />
                                    </div>
                                ))}
                            </div>
