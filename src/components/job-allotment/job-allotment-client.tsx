...
import { useWorkspace, type Printer, type PrinterStatus, type PrinterTechnology } from '../../hooks/workspace'
...
export function JobAllotmentClient() {
    ...
    const filteredJobs = useMemo(() => {
        ...
        return unassignedJobs.filter((job: any) => job.requiredTechnology === techFilter);
    }, [unassignedJobs, techFilter]);
    
    const filteredMachines = useMemo(() => {
        return machines.filter((machine: Printer) => {
        ...
        <div className="flex-1 p-4 md:p-6 lg:p-8 space-y-8">
            ...
            <ProjectQueue
                ...
                onAssign={(jobId, machineId) => {
                    const job = unassignedJobs.find((j: any) => j.id === jobId);
            ...
            {selectedPrinterForLog && (
                <PrinterLogModal
                    ...
                    jobs={schedule.find((s: any) => s.printerId === selectedPrinterForLog!.id)?.jobs || []}
                />
            )}
        </div>
    );
