...
const jobSchema = z.object({
  ...
  estHoursPerItem: z.number().min(0, 'Hours must be non-negative'),
  estMinutesPerItem: z.number().min(0, 'Minutes must be 60 or less').max(59, 'Minutes must be 60 or less'),
...
const MaterialConfigBlock = ({ groupIndex, materialIndex, removeMaterial, showRemoveButton, errors }: any) => {
    const { getAvailableMaterialsForTechnology, getAvailableFinishesForMaterial, getAvailableMaterialNames, getMaterialColor } = useWorkspace() as any;
