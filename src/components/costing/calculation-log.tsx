...
import type { LoggedCalculation } from '../../hooks/workspace';
...
export function CalculationLog({ calculations, onDelete, onPreview, onLoad }: CalculationLogProps) {
    ...
    const formatCurrency = (value: number | undefined, currency: keyof typeof currencySymbols | undefined) => {
                                ...
                                calculations.map(log => {
                                    return (
                                        <TableRow key={log.id} onDoubleClick={() => onLoad(log)} className="cursor-pointer">
                                            <TableCell>
                                                <div className="w-16 h-16 border rounded-md flex items-center justify-center bg-muted flex-shrink-0">
                                                    {log.inputs.productImage ? (
                                                        <Image src={log.inputs.productImage} alt={log.inputs.jobName || 'Product image'} width={64} height={64} className="object-cover rounded-md" />
                                                    ) : (
                                            ...
                                            <TableCell className="text-sm text-muted-foreground">{log.createdAt ? format(new Date(log.createdAt), "dd-MM-yy, h:mm a") : 'Date not set'}</TableCell>
                                            <TableCell className="font-semibold">{formatCurrency(log.result.subtotal, log.inputs.currency)}</TableCell>
                                            <TableCell className="font-semibold">{formatCurrency(log.result.consumerPrice, log.inputs.currency)}</TableCell>
                                            <TableCell className="font-semibold">{formatCurrency(log.result.resellerPrice, log.inputs.currency)}</TableCell>
