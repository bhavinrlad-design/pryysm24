...
import type { InventoryItem } from "@/hooks/workspace"

export function InventoryStatus() {
  const { inventory, spools, resins, powders } = useWorkspace() as any;
  const [selectedItem, setSelectedItem] = useState<InventoryItemData | null>(null);

  const rawMaterialItems = useMemo((): InventoryItemData[] => {
    const allMaterials = [
      ...spools.map((s: any) => ({ name: s.name, quantity: spools.filter((sp: any) => sp.name === s.name && sp.status !== 'Empty').length, unit: 'spools', status: 'In Stock', historicalUsage: '10 spools/month' })),
      ...resins.map((r: any) => ({ name: r.name, quantity: resins.filter((re: any) => re.name === r.name && re.status !== 'Empty').length, unit: 'bottles', status: 'In Stock', historicalUsage: '5 bottles/month' })),
      ...powders.map((p: any) => ({ name: p.name, quantity: powders.filter((po: any) => po.name === p.name && po.status !== 'Empty').length, unit: 'kg', status: 'In Stock', historicalUsage: '2 kg/month' })),
    ];
