...
export function StatCards({ currency, revenueTimeframe }: StatCardsProps) {
  const { orders, printers, documents } = useWorkspace() as any
  ...
  const { 
    ...
    const currentOrders = orders.filter((order: any) => isCurrentPeriodOrder(new Date(order.orderDate)));
    const currentInvoices = documents.filter((doc: any) => doc.type === 'Tax Invoice' && isCurrentPeriodInvoice(new Date(doc.date)));     const totalOrders = currentOrders.length;
    const totalRevenue = currentInvoices.reduce((sum: number, doc: any) => sum + doc.amount, 0);
    ...
    const previousOrders = orders.filter((order: any) => isPreviousPeriodOrder(new Date(order.orderDate)));
    const previousInvoices = documents.filter((doc: any) => doc.type === 'Tax Invoice' && isPreviousPeriodInvoice(new Date(doc.date)));

    const previousTotalOrders = previousOrders.length;
    const previousTotalRevenue = previousInvoices.reduce((sum: number, doc: any) => sum + doc.amount, 0);
    ...
    return {
        ...
        ordersChange,
        revenueChange,
        activePrinters: printers.filter((p: any) => p.status === 'printing' || p.status === 'running').length,
