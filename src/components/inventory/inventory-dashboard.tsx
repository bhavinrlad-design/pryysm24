...
import type { InventoryItem, StockStatus, InventoryCategory } from '../../hooks/workspace'
                        ...
                        return (
                            <Card key={category} className="hover:shadow-lg transition-shadow cursor-pointer">
                                <CardContent className="p-6 flex flex-col items-center justify-center text-center">
                                    ...
                                    <p className="text-muted-foreground text-sm">{String(count)} items</p>
                                </CardContent>
                            </Card>
                        )
                    })}
                    ...
                    {itemsNeedingAttention.map(item => (
                        <Card key={item.id} className="relative">
                             <div className={`absolute left-0 top-0 bottom-0 w-1.5 rounded-l-lg 
                                ...
                                ${item.status === 'Out of Stock' ? 'bg-error' : ''}
                                ${item.status === 'Need Reorder' ? 'bg-accent' : ''}
                             `}></div>
                            <CardHeader>
                            ...
                            <CardContent className="flex justify-between items-center pt-0">
                                <div>
                                 ...
                                 <span className={`px-2 py-1 text-xs font-semibold rounded-full
                                    ...
                                    ${item.status === 'Out of Stock' ? 'bg-error/10 text-error' : ''}
                                    ${item.status === 'Need Reorder' ? 'bg-accent/10 text-accent' : ''}
                                `}>
                                    {item.status}
                                </span>
                            </CardContent>
