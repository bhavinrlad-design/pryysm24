...
import type { InventoryItem, StockStatus } from '../../hooks/workspace'
...
interface ReorderableInventoryItem {
    id: string;
    name: string;
    quantity: number;
    minStock: number;
...
export function ReorderManagement({ allInventory }: ReorderManagementProps) {
    ...
    useEffect(() => {
        setReorderList(prevList => {
            const newList = itemsToReorder.map((item: ReorderableInventoryItem) => {
                const existing = prevList.find((p: ReorderableInventoryItem) => p.id === item.id);
