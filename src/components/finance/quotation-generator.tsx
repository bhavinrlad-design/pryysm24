...
import type { Customer, CodeSettings } from '../../hooks/workspace';
import {
  Select,
  SelectContent,
  SelectItem,
...
export function QuotationGenerator({ currency, companyDetails, bankDetails, showCompanyTaxId, idService }: QuotationGeneratorProps) {
  const { customers, addCustomer, updateCustomer } = useWorkspace() as any;
  ...
  const selectedCustomer = React.useMemo(() => customers.find((c: Customer) => c.id === selectedCustomerId), [customers, selectedCustomerId]);
