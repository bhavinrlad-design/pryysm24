...
import type { Spool, Resin, Powder } from '../../hooks/workspace'

// Define missing types
type MaterialStatus = 'available' | 'low' | 'empty' | 'expired' | 'Empty' | 'Low' | 'Critical' | 'New' | 'Active' | 'Need Reorder';
...
const statusBadgeColors: Record<MaterialStatus, string> = {
    ...
    "Empty": "bg-neutral-100 text-neutral-800", 
    "empty": "bg-neutral-100 text-neutral-800",
    "Need Reorder": "bg-accent/10 text-accent",
    ...
    const spoolsToReorder = useMemo(() => {
        ...
        allSpools.forEach(spool => {
            if (spool.status === 'Low' || spool.status === 'Critical' || spool.status === 'Need Reorder') {
                ...
                if (!groups[key]) {
                    groups[key] = { key, name: spool.name, brand: spool.brand, material: spool.material, color: spool.color, finish: spool.finish, lowStockCount: 0, reorderQty: spool.minOrder || 5, status: 'Low', imageUrl: spool.imageUrl };
                }
    ...
    const resinsToReorder = useMemo(() => {
        ...
        allResins.forEach(resin => {
            if (resin.status === 'Low' || resin.status === 'Critical' || resin.status === 'Need Reorder') {
                ...
                if (!groups[key]) {
                    groups[key] = { key, name: resin.name, brand: resin.brand, type: resin.type, color: resin.color, lowStockCount: 0, reorderQty: resin.minOrder || 5, status: 'Low', imageUrl: undefined };
                }
                groups[key].lowStockCount++;
    ...
    const powdersToReorder = useMemo(() => {
        ...
        allPowders.forEach(powder => {
            if (powder.status === 'Low' || powder.status === 'Critical' || powder.status === 'Need Reorder') {
                ...
                if (!groups[key]) {
                    groups[key] = { key, name: powder.name, brand: powder.brand, material: powder.material, color: powder.color || '#FFFFFF', lowStockCount: 0, reorderQty: powder.minOrder || 5, status: 'Low', imageUrl: undefined };
                }
                groups[key].lowStockCount++;
