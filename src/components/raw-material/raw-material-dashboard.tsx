...
const currencySymbols: Record<Currency, string> = { 'USD': '$', 'EUR': '€', 'GBP': '£', 'AED': 'AED', 'INR': '₹' };
                                ...
                                {itemsNeedingAttention.length > 0 ? itemsNeedingAttention.map((item, index) => {
                                    ...
                                    return (
                                        <TableRow key={`${item.itemType}-${item.id}`}>
                                            ...
                                            <TableCell>
                                                                                                <Badge className={statusBadgeColors[item.status as MaterialStatus] || "bg-neutral-100 text-neutral-800"}>{item.status}</Badge>
                                            </TableCell>
                                            <TableCell className="text-right">
                                                <Button 
                                                    variant="outline" 
                                                    size="sm" 
                                                    onClick={() => onReorderRequest(item.itemType, Number(item.id))}
