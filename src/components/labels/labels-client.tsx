...
import type { Printer as PrinterType, Spool, Resin, Powder, InventoryItem } from '../../hooks/workspace';
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
export function LabelsClient() {
    const { orders, customers, printers, schedule, spools, resins, powders, inventory } = useWorkspace() as any;
                ...
                <TabsContent value="project">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start mt-6">
                        ...
                        <div className="lg:col-span-2">
                            <LabelPreviewContainer title="Project Label Previews" description={selectedOrder ? `Showing ${typeof selectedOrder.items === 'number' ? selectedOrder.items : (Array.isArray(selectedOrder.items) ? selectedOrder.items.length : 1)} label(s) for order ${selectedOrder.orderNumber}.` : 'Select an order to see previews.'}>
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4" ref={labelContainerRef}>
                                    {selectedOrder ? (
                                        Array.from({ length: typeof selectedOrder.items === 'number' ? selectedOrder.items : (Array.isArray(selectedOrder.items) ? selectedOrder.items.length : 1) }).map((_, index) => (
                                            <div data-label-type="project" key={`preview-${selectedOrder.id}-${index}`}>
                                                <ProjectLabel order={selectedOrder} currentItem={index + 1} totalItems={typeof selectedOrder.items === 'number' ? selectedOrder.items : (Array.isArray(selectedOrder.items) ? selectedOrder.items.length : 1)} />
                                            </div>
                                        ))
