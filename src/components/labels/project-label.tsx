...
export const ProjectLabel = forwardRef<HTMLDivElement, ProjectLabelProps>(({ order, currentItem, totalItems }, ref) => {
  ...
  const { customers, printers, schedule } = useWorkspace() as any;
  const customerName = typeof order.customer === 'string' ? order.customer : (order.customer && typeof order.customer === 'object' ? order.customer.name || '' : '');
  ...
  const assignedPrinter = useMemo(() => {
    ...
    for (const printerSchedule of schedule) {
      if (!printerSchedule.jobs || !Array.isArray(printerSchedule.jobs)) continue;
      const job = printerSchedule.jobs.find((j: any) => j.projectCode === order.projectCode);
      if (job) {
        return printers.find((p: any) => p.id === printerSchedule.printerId);
      }
    }
    return null;
